...
#pragma omp parallel //num_threads(NUM_THREADS)	//command to use multithread
		{
			...
			for(i = id ,sum = 0; i<num_steps;i+=nthreads)	//for loop to assign rectangles by thread number and calculate every rectangle height
			{
