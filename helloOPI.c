...
#pragma omp parallel //num_threads(NUM_THREADS)	//command to use multithread
		{
			...
			for(i = id ,sum[id][0]=0; i<num_steps;i+=nthreads)	//for loop to assign rectangles by thread number and calculate every rectangle height
			{

				x = (i+0.5)*step;	//calculate the x-vlaue of rectangle assigned to different threads
				sum[id][0] += 4.0/(1.0+x*x);	//calculate the height of rectanlge assigned to different threads and add them tgz
			}
		}
		int i;
		for(i = 0; i<nthrds; i++)pi += sum[i][0]*step;	// for every thread, calculate the size of assigned area and add them tgz
		end = omp_get_wtime();	//get end time
		printf("%d threads takes %f seconds to process, result is %f\n", n, end-start, pi);
	}
	return 0;
}
