...
#pragma omp parallel //num_threads(NUM_THREADS)	//command to use multithread
		{
			...
			sum = sum*step;	// for every thread, calculate the size of assigned area and add them tgz

#pragma omp atmoic
			pi += sum;
		}
		end = omp_get_wtime();	//get end time
		printf("%d threads takes %f time to process with omp atmoic, result is %f\n", n, end-start, pi);
	}
	return 0;
}
